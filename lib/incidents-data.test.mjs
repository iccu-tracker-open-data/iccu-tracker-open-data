...
test("buildSnapshot publishes rows with explicit public flags only", () => {
  const snapshot = buildSnapshot(
    [
      {
        Timestamp: "2026-03-27 18:00:00",
  ...
  assert.equal(snapshot.meta.publishRule, "public_flag_only");
});

test("buildSnapshot does not publish rows when public flag is absent", () => {
  const snapshot = buildSnapshot(
    [
      {
        Timestamp: "2026-03-27 18:00:00",
